...
/*Open the training AND test files using the paths written in the ocr.h file*/
int    open_files(t_fds *fds)
{
    ssize_t n;
    ssize_t i = 0;
    ssize_t j = 0;
    ...
    //skip the header of the images file
...
void    close_train_fds(t_fds *fds)
{
    close(fds->fd_labels);
    close(fds->fd_images);
}

void    close_test_fds(t_fds *fds)
{
    close(fds->fd_test_labels);
